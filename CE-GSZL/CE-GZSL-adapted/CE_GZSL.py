...
parser.add_argument('--split', default='_0',help='In case of LAD')
...
for epoch in range(opt.nepoch):
    ...
    cls = classifier_embed_contras.CLASSIFIER(opt.dataset, train_X, train_Y, netMap, opt.embedSize, data, nclass, opt.cuda,
                                              opt.classifier_lr, opt.split, 0.5, 25, opt.syn_num,
                                              True)
    ...
    if cls.H > best_gzsl:
        ...
        #syn_feature, syn_label = generate_syn_feature(netG, data.unseenclasses, data.attribute, opt.syn_num)
    cls = classifier_embed_contras.CLASSIFIER(opt.dataset, syn_feature, util.map_label(syn_label, data.unseenclasses), netMap,
                                              opt.embedSize, data,
                                              data.unseenclasses.size(0), opt.cuda, opt.classifier_lr, opt.split, 0.5, 100,
                                              opt.syn_num,
                                              False)
    acc = cls.acc
...
print(f"[ZSL] Top-1 Accuracy (%): {best_zsl:.2f} %")
print(f"[GSZL] Accuracy (%) - Seen: {best_acc_seen:.2f} %, Unseen: {best_acc_seen:.2f} %, Harmonic: {best_gzsl:.2f} %")