...
for epoch in range(0,opt.nepoch):
    ...
    if opt.gzsl:
        ...
        gzsl_cls = classifier.CLASSIFIER(train_X, train_Y, data, nclass, opt.cuda, opt.classifier_lr, opt.split, 0.5, \
    ...
    zsl_cls = classifier.CLASSIFIER(syn_feature, util.map_label(syn_label, data.unseenclasses), \
                    data, data.unseenclasses.size(0), opt.cuda, opt.classifier_lr, opt.split, 0.5, 25, opt.syn_num, \
...
print(f'[ZSL] Top-1 Accuracy (%) - {best_zsl_acc:.2f} %')
if opt.gzsl:
    print(f'[GZSL] Accuracy (%) - Seen: {best_acc_seen:.2f} %, Unseen: {best_acc_unseen:.2f} %, Harmonic: {best_gzsl_acc:.2f} %')