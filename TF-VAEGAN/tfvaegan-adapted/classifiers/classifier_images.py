"""
NOTE:
1. This code was originally created by the author of the GitHub repository https://github.com/akshitac8/tfvaegan
...
class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, dataset, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, split='_0', _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True, netDec=None, dec_size=4096, dec_hidden_size=4096):
    ...
    def fit_zsl(self):
        ...
        if self.dataset == "LAD":
            np.savetxt("preds_TFVAEGAN_zsl_att"+str(self.split)+".txt", self.preds_zsl[pos])
    ...
    def fit(self):
        ...
        if self.dataset == "LAD":
            np.savetxt("preds_seen_TFVAEGAN_gzsl_att" + str(self.split) + ".txt", self.preds_seen_gzsl[pos])
            np.savetxt("preds_unseen_TFVAEGAN_gzsl_att" + str(self.split) + ".txt", self.preds_unseen_gzsl[pos])
